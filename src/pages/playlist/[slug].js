...
export async function getServerSideProps(context) {
  const playlistID = context.query.slug
  let songs = (await pool.query("SELECT songs FROM playlists where playlistID=$1",[playlistID])).rows[0].songs;
  let playlists = (await pool.query("SELECT * FROM playlists")).rows;
  let songs2=[]
  for(let song in songs){
    //console.log(song)
    songs2.push((await pool.query("SELECT * FROM songs where songID=$1",[songs[song]])).rows[0])
  }
  //console.log(songs2);
  return {
    props: { playsongs: JSON.parse(JSON.stringify(songs2)),playlists:JSON.parse(JSON.stringify(playlists)) },
  };
}
